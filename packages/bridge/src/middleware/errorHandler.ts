...
export const bridgeValidationError = (message = 'Bridge validation failed') => {
  return new ApiError(422, message);
};

export const bridgeProcessingError = (message = 'Bridge transaction processing failed') => {
  return new ApiError(500, message, true);
};

export const insufficientLiquidityError = (message = 'Insufficient bridge liquidity') => {
...
export const errorHandler = (err: Error, req: Request, res: Response, next: NextFunction) => {
  ...
  if (err instanceof ApiError) {
    ...
    statusCode = 503;
    message = err.message || 'Blockchain transaction failed';
    isOperational = true;
  }
